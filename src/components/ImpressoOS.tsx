...
export function ImpressoOS({ os, itens }: ImpressoOSProps) {
  ...
  const dataAbertura = os.data_abertura ? new Date(os.data_abertura).toLocaleDateString('pt-BR') : '';
  const totalServicos = itens.filter(i => i.tipo === 'SERVICO').reduce((acc, i) => acc + Number(i.subtotal || 0), 0);
  const totalPecas = itens.filter(i => i.tipo === 'PECA').reduce((acc, i) => acc + Number(i.subtotal || 0), 0);
  ...
  return (
    <div className="hidden print:block fixed inset-0 bg-white z-[9999] p-8 text-black">
      ...
      <div className="flex justify-between items-start border-b-2 border-black pb-4 mb-6">
        <div>
            <h1 className="text-4xl font-bold uppercase tracking-wider">VR Diesel</h1>
        ...
        <div className="text-right">
            ...
            <p className="text-sm mt-2 font-bold">Data: {dataAbertura}</p>
        ...
        <tbody>
            {itens.map((item, idx) => (
                <tr key={idx} className="border-b border-gray-300">
                    ...
                    <td className="p-2 border border-gray-300 text-right">{formatMoney(item.preco_un || 0)}</td>
                    <td className="p-2 border border-gray-300 text-right font-bold">{formatMoney(item.subtotal || 0)}</td>
                </tr>
            ))}
        </tbody>
      </table>
      ...
      <div className="flex justify-end mb-12">
        <div className="w-64 space-y-2">
            ...
            <div className="flex justify-between text-xl font-bold border-t-2 border-black pt-2">
                ...
                <span>{formatMoney(os.total || 0)}</span>
            </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-4 left-0 right-0 text-center text-[10px] text-gray-400 print:block hidden">
        Sistema VR Diesel - Gerado em {new Date().toLocaleString('pt-BR')}
      </div>
    </div>
  );
}
